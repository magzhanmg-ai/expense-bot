...
def save_expense(amount, category, description):
    try:
        sheet = get_sheet()
        sheet.append_row([datetime.now().strftime("%d.%m.%Y"), amount, category, description])
        return True
    except Exception as e:
...
Когда пользователь называет расход — отвечай ТОЛЬКО JSON без пояснений:
    ...
    try:
        if reply.strip().startswith("{"):
            ...
            if data.get("action") == "save_expense":
                ...
                msg = f"✅ Бюджет: {data['amount']:.0f}р сохранён! 📋"
                await update.message.reply_text(msg)
                chat_histories[user_id].append({"role": "assistant", "content": msg})
                return
    except Exception:
